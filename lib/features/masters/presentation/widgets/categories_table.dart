...
class CategoriesTable extends ConsumerStatefulWidget {
  const CategoriesTable({super.key});

  @override
  ConsumerState<CategoriesTable> createState() => _CategoriesTableState();
}

class _CategoriesTableState extends ConsumerState<CategoriesTable> {
        ...
        children: [
          ...
          categoriesAsync.when(
            ...
            error: (error, _) => Padding(
              ...
              child: Center(child: Text('Could not load categories: $error', style: AppTypography.bodyMuted)),
            ),
            data: (categories) {
              ...
              if (filtered.isEmpty) {
                return Padding(
                  padding: const EdgeInsets.symmetric(vertical: AppSpacing.lg),
                  child: Center(child: Text('No categories match "$query".', style: AppTypography.bodyMuted)),
                );
              }
                  ...
                  child: Column(
                    children: [
                      ...
                      for (final category in filtered)
                        _dataRow(category.name, category.description ?? '—', browser.current?.name == category.name),
                    ],
                  ),
                ),
              );
  ...
  Widget _dataRow(String name, String description, bool isSelected) {
    ...
    return InkWell(
      onTap: () => ref.read(categoryBrowserProvider.notifier).selectByName(name),
      child: Container(
